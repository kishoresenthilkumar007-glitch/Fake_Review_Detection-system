...
interface BatchPredictResult {
    ...
    results_preview: ResultPreviewRow[];
}
...
let processedFileId: string | null = null;
...
(function init3DBackground() {
    ...
    // Mouse interactivity
    const mouse: MousePos = { x: 0, y: 0, targetX: 0, targetY: 0 };
...
// Tab Switching
// ============================================================================
function switchTab(tabName: 'single' | 'batch'): void {
    ...
    setTimeout(() => {
        document.querySelectorAll('.tab-content.active .tilt-card').forEach((el: Element) => {
            const card = el as HTMLElement;
            card.addEventListener('mousemove', (e: MouseEvent) => {
                const rect = card.getBoundingClientRect();
                const x = e.clientX - rect.left;
...
// Single Review logic
// ============================================================================
const reviewTextarea = document.getElementById('review-input') as HTMLTextAreaElement | null;
const charCounter = document.getElementById('char-counter') as HTMLElement | null;

if (reviewTextarea && charCounter) {
    reviewTextarea.addEventListener('input', (e: Event) => {
...
function clearSingleInput(): void {
    if (reviewTextarea && charCounter) {
...
function hideSingleResult(): void {
...
async function analyzeSingleReview(): Promise<void> {
    ...
    const btn = document.getElementById('btn-analyze-single') as HTMLButtonElement | null;
    if (!btn) return;
    const btnText = btn.querySelector('.btn-text') as HTMLElement;
    const btnIcon = btn.querySelector('.btn-icon') as HTMLElement;
    const loader = btn.querySelector('.loader') as HTMLElement;
    ...
    try {
        ...
        const data = await response.json() as SinglePredictResult;
        ...
        if (response.ok) {
            ...
            showToast((data as any).error || 'An error occurred during analysis.', 'error');
        }
    } catch (err) {
        console.error(err);
        if (scanOverlay) scanOverlay.classList.add('hidden');
...
function displaySingleResult(data: SinglePredictResult): void {
    const resultCard = document.getElementById('single-result');
    ...
    const gaugeCircle = document.getElementById('gauge-fill-circle') as SVGCircleElement | null;
...
// ============================================================================
function animateNumber(el: HTMLElement, start: number, end: number, duration: number, suffix = ''): void {
...
function animateIntNumber(el: HTMLElement, start: number, end: number, duration: number): void {
    const startTime = performance.now();
    function step(currentTime: number) {
        const elapsed = currentTime - startTime;
        const progress = Math.min(elapsed / duration, 1);
...
async function scanFileColumns(file: File): Promise<void> {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const analyzeBtn = document.getElementById('btn-analyze-batch') as HTMLButtonElement | null;
            if (analyzeBtn) {
...
function populateColumnDropdown(columns: string[], selectedCol: string): void {
    const select = document.getElementById('column-select') as HTMLSelectElement | null;
    if (!select) return;
    select.innerHTML = '';
...
async function analyzeBatchFile(): Promise<void> {
    if (!selectedFile) return;

    const btn = document.getElementById('btn-analyze-batch') as HTMLButtonElement | null;
    if (!btn) return;
    const btnText = btn.querySelector('.btn-text') as HTMLElement;
    const btnIcon = btn.querySelector('.btn-icon') as HTMLElement;
    const loader = btn.querySelector('.loader') as HTMLElement;
    ...
    try {
        ...
        const data = await response.json() as BatchPredictResult;
        if (response.ok) {
            displayBatchResults(data);
        } else {
            showToast((data as any).error || 'An error occurred during dataset analysis.', 'error');
        }
    } catch (err) {
        console.error(err);
        showToast('Failed to connect to backend server.', 'error');
...
function displayBatchResults(data: BatchPredictResult): void {
    ...
    if (totalEl) animateIntNumber(totalEl, 0, data.total_rows, 1500);
    if (genuineEl) animateIntNumber(genuineEl, 0, data.truthful_count, 1500);
    if (deceptiveEl) animateIntNumber(deceptiveEl, 0, data.deceptive_count, 1500);
    ...
    if (genuinePctEl) genuinePctEl.textContent = `${genuinePct}%`;
    if (deceptivePctEl) deceptivePctEl.textContent = `${deceptivePct}%`;
...
// Chart Configuration
// ============================================================================
function renderDistributionChart(genuine: number, deceptive: number): void {
    const canvas = document.getElementById('distributionChart') as HTMLCanvasElement | null;
    if (!canvas) return;
...
function downloadProcessedResults(): void {
...
function handleMobileNav(tabName: 'single' | 'batch'): void {
    switchTab(tabName);
...
(window as any).handleMobileNav = handleMobileNav;
(window as any).switchTab = switchTab;
(window as any).clearSingleInput = clearSingleInput;
(window as any).analyzeSingleReview = analyzeSingleReview;
(window as any).clearSelectedFile = clearSelectedFile;
(window as any).analyzeBatchFile = analyzeBatchFile;
(window as any).downloadProcessedResults = downloadProcessedResults;
